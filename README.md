...
- 全市场股票池（东方财富 + BaoStock + AKShare 三源，point-in-time 不可变快照，含 ST/新股/退市/停牌排除；「股票池」页可浏览全量标的、按市场与状态过滤并手动触发同步）
...
6. 「股票池」页可确认本地池子到底有多少只标的（沪深京三市约 5900 只）、最近一次同步时间、
   哪些被剔除（ST / 退市 / 停牌），并手动触发同步；「技术指标」页可查任意标的的
...
回归基线：后端 757 项测试全绿、前端类型检查与生产构建零错误、managed E2E 19/19。
