...
- **首页「赚钱率前五」（2026-09-15）**：`GET /api/realtime/win-rate` + 看板面板。
  对当前实时排名前 N 名的候选，用本地**前复权日线逐日回放**「同类条件」（每天只用当天及
  以前的数据，无未来函数），统计持有 `hold_days` 个交易日（默认 5 日，T+1 收盘买入）的
  **样本内胜率 / 平均收益 / 中位数 / 样本数**，据此排出前五；页面每 30 秒~5 分钟自动重算，
  也可手动重算。样本数不足 5 次的候选**不参与排名**（进 `skipped`），缺样本不编数字。
  ⚠️ 该指标是**样本内回放胜率，不是未来上涨概率**，未通过样本外验证、未扣交易费用，
  只能用来排序研究优先级；接口与页面都带这条声明（回放只用 5 条可由日线复算的条件，
  与实时选股的 8 条不完全相同，响应里写明用了哪几条）。
  实测（2026-09-15 盘中）：一次调用 18.0 秒，行情覆盖 100%，前五赚钱率 60.0%/57.8%/55.0%/54.2%/50.0%，
  样本数 30/83/80/24/58。
- **投资研究（2026-09-14 新增，面向个人本机自用）**：
  ...
  - 研究记录与**复核**：`POST /api/research/runs` 由服务端重算并冻结数据时点、全部假设、
    统一分析、反向估值、双审查与 SHA-256 指纹（记录**不可修改**，新观点请建新记录）；
    `GET /api/research/runs/{id}/review` 用**当前**数据重算并回答"要不要重新研究"——
    列出 5 个复核触发条件（新财报报告期 / 现价相对基准价值偏离 ≥ ±20% / 结论状态变化 /
    报告期滞后 > 270 天 / 记录超过 30 天未更新）以及与上一条记录的逐项差异
    （含现价、质量分、置信度、各情景价值、假设项、正反证据与失效条件的新增/移除）。
    前端「投资研究」页的历史记录里点「复核」即可查看；复核**只读**，不会覆盖任何记录。
  - 研究结论到组合草案：`POST /api/research/runs/{id}/rebalance-draft` 把冻结结论映射成
    **只读模拟增持草案**。单股上限、行业上限、单笔损失预算、止损距离、组合回撤预算、
    相关性上限与成交额参与率都由使用者显式填写；相关收益样本少于 20 期、行业/流动性数据
    缺失或结论不是 `attractive` 时禁止增加仓位。接口不写订单，执行仍须走模拟盘人工确认门禁。
