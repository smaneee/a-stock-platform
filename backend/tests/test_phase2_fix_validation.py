...
class TestE2EManagedConstraints:
    """Managed E2E 模式禁止硬编码绝对项目路径或特定 venv 路径。"""

    def test_e2e_smoke_uses_path_file_relative_root(self):
        """e2e_smoke.py 必须用 Path(__file__).resolve().parents[1] 派生 REPO_ROOT，
        不能出现硬编码绝对路径。"""
        from pathlib import Path as _P

        repo_root = _P(__file__).resolve().parents[2]
        e2e_path = repo_root / "scripts" / "e2e_smoke.py"
        content = e2e_path.read_text(encoding="utf-8")

        # 禁止硬编码绝对项目根路径（含盘符 + 路径分隔）
        forbidden = [
            "E:/workbuddy work/",
            "E:\\workbuddy work\\",
            "C:\\workbuddy",
        ...
        for token in forbidden:
            assert token not in content, (
                f"e2e_smoke.py 含硬编码绝对路径 '{token}'，必须改用 REPO_ROOT 派生"
            )

    def test_e2e_smoke_uses_sys_executable_not_venv_hardcode(self):
        """e2e_smoke.py 必须用 sys.executable 调子进程，不能硬编码 .venv 路径。"""
        from pathlib import Path as _P
        ...
        for token in forbidden:
            assert token not in content, (
                f"e2e_smoke.py 含硬编码 venv 路径 '{token}'，必须改用 sys.executable"
            )

        # 必须使用 sys.executable
        assert "sys.executable" in content, (
            "e2e_smoke.py 必须使用 sys.executable 调 alembic / uvicorn"
        )

    def test_main_managed_init_inside_try_finally(self):
        """init_temp_db 必须在 main_managed 的 try/finally 块内调用。"""
        from pathlib import Path as _P
        ...
        # 定位 main_managed 函数的范围
        idx = content.find("async def main_managed")
        assert idx >= 0, "未找到 main_managed"
        # 找到函数末尾（下一个顶级 def）
        end_marker = "\nasync def main_external"
        end_idx = content.find(end_marker, idx)
        assert end_idx > idx, "未找到 main_managed 结尾"
        managed_body = content[idx:end_idx]

        # main_managed 内必须包含 try/finally + init_temp_db + 进程启动
        assert "try:" in managed_body
        assert "finally:" in managed_body
        assert "init_temp_db" in managed_body
        # 三个清理路径关键词：kill_process（后端/前端）、delete_temp_db、verify_no_residue
        # 都必须出现在 finally 块中（用一个宽松的位置检查：都在 try: 后面）
        try_idx = managed_body.find("try:")
        finally_idx = managed_body.find("finally:")
        # finally 块范围
        finally_block = managed_body[finally_idx:]
        assert "delete_temp_db" in finally_block
        assert "kill_process" in finally_block
        assert "verify_no_residue" in finally_block

    def test_a_stock_db_protection_in_managed(self):
        """managed 模式必须拒绝与 a_stock.db 路径冲突的临时 DB。"""
        from pathlib import Path as _P
        ...
        # 必须有 a_stock.db 路径检查
        assert "A_STOCK_DB" in content
        assert (
            "拒绝使用 a_stock.db" in content
            or "与 a_stock.db 冲突" in content
        ), "managed 模式必须有 a_stock.db 路径保护"

    def test_a_stock_db_unchanged_check_in_managed(self):
        """managed 模式必须校验 a_stock.db 在 E2E 期间未被修改。"""
        from pathlib import Path as _P
        ...
        # 必须有 a_stock.db 修改校验函数
        assert "_verify_a_stock_db_unchanged" in content, (
            "managed 模式必须有 a_stock.db 修改校验"
        )

    def test_repo_root_derived_from_script_path(self):
        """REPO_ROOT 必须用 Path(__file__).resolve().parents[1] 派生。"""
        from pathlib import Path as _P
        ...
        # 必须使用 parents[N] 派生
        assert (
            "Path(__file__).resolve().parents[1]" in content
        ), "REPO_ROOT 必须通过 Path(__file__).resolve().parents[1] 派生"